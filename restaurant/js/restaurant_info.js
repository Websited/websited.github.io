    ...
    fetchRestaurantFromURL((error, restaurant) => {
        if (error) { // Got an error!
            ...
            L.tileLayer('https://api.tiles.mapbox.com/v4/{id}/{z}/{x}/{y}.jpg70?access_token={mapboxToken}', {
                mapboxToken: 'pk.eyJ1IjoidGlja21laXN0ZXIiLCJhIjoiY2pqdTdhNXRxMGIxNDNzb2MzbnI4NTlnNyJ9.Ntfpt9NqwXd6SiI6M8JPLQ',
                maxZoom: 18,
...
createReviewHTML = (review) => {
    ...
    li.setAttribute('tabindex', (self.restaurant.reviews.indexOf(review) + 8));
    const revHead = document.createElement('div');
    revHead.classList.add('review-header');
    li.appendChild(revHead);

    const name = document.createElement('p');
    name.classList.add('review-name');
    name.innerHTML = review.name;
    revHead.appendChild(name);

    const date = document.createElement('p');
    date.innerHTML = review.date;
    date.classList.add('review-date');
    revHead.appendChild(date);
